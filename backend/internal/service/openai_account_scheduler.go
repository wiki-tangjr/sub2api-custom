...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if s.service.shouldBypassOpenAIImageSticky(req, accountID) {
		return nil, false, nil
	}
	...
	result, acquireErr := s.service.tryAcquireAccountSlot(ctx, accountID, s.service.openAIImageEffectiveSlotConcurrency(req, account))
	...
	if s.service.concurrencyService != nil {
		...
		return attachSelectionProfitGate(ctx, &AccountSelectionResult{
			...
			WaitPlan: &AccountWaitPlan{
				AccountID:      accountID,
				MaxConcurrency: s.service.openAIImageEffectiveSlotConcurrency(req, account),
...
type openAIAccountCandidateScore struct {
	...
	ttft      float64
	hasTTFT   bool
	image     openAIImageAccountHealth
}
...
func isOpenAIAccountCandidateBetter(left openAIAccountCandidateScore, right openAIAccountCandidateScore) bool {
	...
	if left.image.slow != right.image.slow {
		return !left.image.slow
	}
	if left.image.recentP95Ms != right.image.recentP95Ms {
		if left.image.recentP95Ms <= 0 {
			return false
		}
		if right.image.recentP95Ms <= 0 {
			return true
		}
		return left.image.recentP95Ms < right.image.recentP95Ms
	}
...
func (s *defaultOpenAIAccountScheduler) buildOpenAIAccountLoadPlan(
	...
	for _, account := range filtered {
		...
		imageHealth := openAIImageAccountHealth{}
		if s.service != nil {
			imageHealth = s.service.openAIImageAccountHealth(req, account.ID)
		}
	...
	for i := range candidates {
		...
		if item.image.applicable {
			item.score += item.image.scoreAdjustment
		}
		if req.StickyWeighted {
...
func (s *defaultOpenAIAccountScheduler) buildOpenAISelectionOrder(
	...
	rankSelectionOrder := func(pool []openAIAccountCandidateScore) []openAIAccountCandidateScore {
	...
	buildSelectionOrder := func(pool []openAIAccountCandidateScore) []openAIAccountCandidateScore {
		...
		if req.ImageEndpoint != "" {
			...
			if len(healthy) > 0 {
				ordered := rankSelectionOrder(healthy)
				if len(slow) > 0 {
					ordered = append(ordered, rankSelectionOrder(slow)...)
				}
				return ordered
			}
		}
		return rankSelectionOrder(pool)
	}
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrderWithBudget(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		freshConcurrency := fresh.Concurrency
		if s.service != nil {
			freshConcurrency = s.service.openAIImageEffectiveSlotConcurrency(req, fresh)
		}
		if freshConcurrency != candidate.account.Concurrency {
			release(result)
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	for i := range accounts {
		...
		// LOCAL CUSTOMIZATION: Image account cooling check
		if s.service.isOpenAIImageAccountCooling(req, account.ID) {
			continue
		}
		...
		if s.service != nil {
			maxConcurrency = s.service.openAIImageEffectiveLoadConcurrency(req, account)
		}
	...
	for pass := 0; pass < passes; pass++ {
		...
		for _, candidate := range attempt.selectionOrder {
			...
			return attachSelectionProfitGate(ctx, &AccountSelectionResult{
				...
				WaitPlan: &AccountWaitPlan{
					AccountID:      fresh.ID,
					MaxConcurrency: s.service.openAIImageEffectiveSlotConcurrency(req, fresh),
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForImages(
	...
	selection, decision, err := s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, imageEndpoint, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", requiredCapability, false, PlatformOpenAI, false, false)
	if err == nil && selection != nil && selection.Account != nil {
		return selection, decision, nil
	}
	// 如果要求 native 能力（如指定了模型）但没有可用的 APIKey 账号，回退到 basic（OAuth 账号）
	if requiredCapability == OpenAIImagesCapabilityNative {
		return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, imageEndpoint, excludedIDs, OpenAIUpstreamTransportHTTPSSE, "", OpenAIImagesCapabilityBasic, false, PlatformOpenAI, false, false)
	}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	selection, decision, err := s.selectAccountWithSchedulerOnce(ctx, groupID, previousResponseID, sessionHash, requestedModel, imageEndpoint, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
	...
	return s.selectAccountWithSchedulerOnce(withOpenAIProxyStreamQuarantineBypass(ctx), groupID, previousResponseID, sessionHash, requestedModel, imageEndpoint, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
}
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	sessionHash string,
	requestedModel string,
	// 本地魔改 #3：图片延迟感知调度需要 imageEndpoint 透传到底层选号。
	imageEndpoint string,
	excludedIDs map[int64]struct{},
	requiredTransport OpenAIUpstreamTransport,
	requiredCapability OpenAIEndpointCapability,
	...
	if scheduler == nil {
		...
		if requiredTransport == OpenAIUpstreamTransportAny || requiredTransport == OpenAIUpstreamTransportHTTPSSE {
			...
			for {
				selection, err := s.selectAccountWithLoadAwareness(ctx, groupID, platform, legacySessionHash, requestedModel, effectiveExcludedIDs, requireCompact, requiredCapability, imageEndpoint, useUpstreamTokenCost)
				if err != nil {
					return nil, decision, err
				}
				if selection == nil || selection.Account == nil {
