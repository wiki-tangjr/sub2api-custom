...
func RegisterGatewayRoutes(
	...
	// LOCAL CUSTOMIZATION: OpenAI-compatible video/Jimeng proxy plus upstream Grok video support.
	// A wildcard dispatcher is required because Gin cannot register the upstream named
	// /videos routes alongside the custom /videos/*subpath compatibility route.
	...
	r.Any("/videos", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, videoHandler)
	r.Any("/videos/*subpath", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, videoHandler)
	...
	r.Any("/jimeng/*subpath", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, openAIJimengRootHandler)
	...
	r.POST("/seedance/v3/contents/generations/tasks", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, seedanceCreate)
	r.GET("/seedance/v3/contents/generations/tasks/*taskId", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, seedanceQuery)
