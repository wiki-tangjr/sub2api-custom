...
func RegisterGatewayRoutes(
	...
	*/
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		openAIJimengHandler := func(c *gin.Context) {
			if getGroupPlatform(c) != service.PlatformOpenAI {
				service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
				c.JSON(http.StatusNotFound, gin.H{"error": gin.H{"type": "not_found_error", "message": "Jimeng API is not supported for this platform"}})
				return
			}
			h.OpenAIGateway.Videos(c)
		}
	...
	// The wildcard dispatcher retains legacy OpenAI/Jimeng routes while routing
	// upstream Grok's named video endpoints (edits, extensions, status, content).
	r.Any("/videos", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, videoHandler)
	r.Any("/videos/*subpath", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, videoHandler)
	...
	r.Any("/jimeng/*subpath", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, openAIJimengRootHandler)

	// LOCAL CUSTOMIZATION: Seedance native protocol translated to the OpenAI video proxy.
	...
	r.POST("/seedance/v3/contents/generations/tasks", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, seedanceCreate)
	r.GET("/seedance/v3/contents/generations/tasks/*taskId", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), groupModelAllowlist, compositeTarget, requireGroupAnthropic, seedanceQuery)
