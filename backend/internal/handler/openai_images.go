...
const (
	openAIImagesRateLimitSameAccountRetries = 2
...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	imageRateLimitRetryCount := make(map[int64]int)
				...
				if errors.As(err, &failoverErr) {
					...
					if failoverErr.StatusCode == http.StatusTooManyRequests {
						if imageRateLimitRetryCount[account.ID] < openAIImagesRateLimitSameAccountRetries {
							...
							retryDelay := openAIImagesRateLimitRetryDelay(failoverErr)
							reqLog.Warn("openai.images.rate_limit_same_account_retry",
								zap.Int64("account_id", account.ID),
								zap.Int("retry_count", imageRateLimitRetryCount[account.ID]),
								zap.Int("retry_limit", openAIImagesRateLimitSameAccountRetries),
						...
						h.gatewayService.ReportOpenAIImageRateLimit(account.ID, parsed.Endpoint, parsed.Model)
					} else {
...
func openAIImagesRateLimitRetryDelay(err *service.UpstreamFailoverError) time.Duration {
